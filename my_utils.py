...
# my_utils.py
# Treehouse Course - Intro to Data Science with Python
# Author: Kat Chuang
...
#find cashmere in description or name
...
#Let’s try using this to save the 
# this uses the latest one with match conditions
#write_two_cols_with_match('data/cashmere.csv', my_improved_csv[1:], 3, 11, True)

def write_two_cols_with_match(filename, data_sample, col1, col2, match_on=False):    
    #check to see if the header exists in the file
    new_array = []
...
#cvs with sorted string (brand, name)
def write_sorted_string(filename, data_sample):
    pass
...
def prices_of(data_sample):
    temp_list = []
    for row in data_sample[1:]:
        price_col = float(row[2])
        temp_list.append(price_col)
    return temp_list
